--- Модуль обработки битовых сигналов
...
do  -- Класс триггер R-TRIG.
  --[[ 
      ...
      Пример вызова:
      r_trig_1 = M.RTrig:new()   -- Создать экземпляр триггера (метод вызывается один раз).
...
do  -- Класс триггер TRIG по переднему/заднему фронту.
    ...
    function M.Trig:new()
        ...
        function obj:upd(clk)  -- Контроллер триггера.
            self.clk = clk == nil and self.clk or clk
            self.q = false
            if self.clk ~= private.clk_last then
                self.q = true
            end
            private.clk_last = self.clk
            return self.q
